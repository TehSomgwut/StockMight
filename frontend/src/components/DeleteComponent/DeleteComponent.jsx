import deleteStyle from './DeleteComponent.module.css'

export default function DeleteComponent({isShow, setIsShow, h3, p, onConfirm}) {
    return(
        <div className={deleteStyle.DeleteComponent} style={isShow ? {display: 'flex'} : {display: 'none'}}>
            <div className={deleteStyle.blank}></div>
            <div className={deleteStyle.wrapper}>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M21.7304 18L13.7304 4C13.556 3.6922 13.303 3.43618 12.9973 3.25806C12.6917 3.07995 12.3442 2.9861 11.9904 2.9861C11.6366 2.9861 11.2892 3.07995 10.9835 3.25806C10.6778 3.43618 10.4249 3.6922 10.2504 4L2.25042 18C2.0741 18.3054 1.98165 18.6519 1.98243 19.0045C1.98321 19.3571 2.0772 19.7032 2.25486 20.0078C2.43253 20.3124 2.68757 20.5646 2.99411 20.7388C3.30066 20.9131 3.64783 21.0032 4.00042 21H20.0004C20.3513 20.9996 20.6959 20.907 20.9997 20.7313C21.3035 20.5556 21.5556 20.3031 21.7309 19.9991C21.9062 19.6951 21.9985 19.3504 21.9984 18.9995C21.9983 18.6486 21.9059 18.3039 21.7304 18Z" stroke="#FF0000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                <path d="M12 9V13" stroke="#FF0000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                <path d="M12 17H12.01" stroke="#FF0000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>

                <div className={deleteStyle.content}>
                    <h3>{h3}</h3>
                    <p>{p}</p>
                    <div className={deleteStyle["button-container"]}>
                        <button type="submit" onClick={onConfirm}>ยืนยันลบ</button>
                        <button type="reset" onClick={() => setIsShow(!isShow)}>ยกเลิก</button>
                    </div>
                </div>
            </div>
        </div>
    )
}