...
export default function Product({ name, image, metric, code, category, quantity, minStock, status, _id }) {
    ...
    async function onConfirm() {
        try {
            const res = await fetch(`https://stockmight-backend.onrender.com/api/product/${_id}`, { method: 'DELETE' });
            if (res.ok) {
                setIsShow(false);
    ...
    return (
        <div className={styleP.Product}>
            <div className={styleP["product-name"]}>
                {/* เช็ค Path รูปภาพ */}
                <img 
                    src={image && image !== "/" ? `https://stockmight-backend.onrender.com${image}` : '/Icon/2-Inventory/Icon.svg'} 
                    alt={name} 
                />
                <div>
                    <h5>{name}</h5>
